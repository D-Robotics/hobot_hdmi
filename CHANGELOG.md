# Changelog for package hobot_hdmi

tros_2.4.2rc1 (2024-11-26)
------------------
1. 修复RDK X5 OS 3.1.0版本Connector Id变更问题。
2. 修改dma buffer不会自动刷新的问题。

tros_2.4.2 (2024-11-15)
------------------
1. 依赖的OpenCV版本从3.4.5升级到4.X。

tros_2.4.1 (2024-09-18)
------------------
1. 新增X5 hdmi 渲染功能支持, 支持mono 1080P 检测、分割、关键点、手势结果渲染。
2. 修复打包失败问题。

tros_2.4.0 (2024-08-13)
------------------
1. 新增X5 hdmi功能支持, 支持mono 1080P 显示。支持 ros/零拷贝 数据传输

tros_2.1.0 (2024-04-01)
------------------
1. 适配ros2 humble零拷贝。
2. 新增中英双语README。
3. 零拷贝通信使用的qos的Reliability由RMW_QOS_POLICY_RELIABILITY_RELIABLE（rclcpp::QoS()）变更为RMW_QOS_POLICY_RELIABILITY_BEST_EFFORT（rclcpp::SensorDataQoS()）。

tros_2.0.0 (2023-05-11)
------------------
1. 更新package.xml，支持应用独立打包
2. 更新应用启动launch脚本

v1.0.1 (2024-06-23)
------------------
1. 新增模块，实现通过 HDMI 显示接收 ROS2 Node 发布的 image msg，支持 bgr8/rgb8/nv12，支持 ros/零拷贝 数据传输
